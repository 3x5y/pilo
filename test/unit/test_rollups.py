...
class TestRollupOp(pilotest.TestCase):
    ...
    def test_fields(self):
        op = RollupOp(
            ...
            base_snapshot="tank@20260101_000000_000000-anchor",
            target_snapshot="tank@20260102_000000_000000-anchor",
            source_dataset="tank",
        )
        self.assertEqual(op.output_path, Path("/out/r.zfs"))
        self.assertEqual(
            op.base_snapshot, "tank@20260101_000000_000000-anchor")
...
class TestRollupFilename(pilotest.TestCase):
    ...
    def test_rollup_filename(self):
        result = rollups.rollup_filename(
            "20260101_000000_000000-anchor",
            "20260102_000000_000000-anchor",
        )
        self.assertEqual(
            result,
    ...
    def test_rollup_filename_same_date(self):
        result = rollups.rollup_filename(
            "20260101_010000_000000-anchor",
            "20260101_020000_000000-anchor",
        )
        self.assertEqual(
            result,
...
class TestDiscoverAnchorChain(pilotest.TestCase):

    @patch("pilo.zfs.list_snapshots", return_value=[])
    def test_no_snapshots(self, mock_list):
        pairs = rollups.discover_anchor_chain("tank")
        self.assertEqual(pairs, [])

    @patch("pilo.zfs.list_snapshots", return_value=[
        "tank@20260101_000000_000000-anchor",
    ])
    def test_one_anchor(self, mock_list):
        pairs = rollups.discover_anchor_chain("tank")
        self.assertEqual(pairs, [])

    @patch("pilo.zfs.list_snapshots", return_value=[
        "tank@20260101_000000_000000-anchor",
        "tank@20260102_000000_000000-anchor",
    ])
    def test_two_anchors_one_pair(self, mock_list):
        pairs = rollups.discover_anchor_chain("tank")
    ...
    @patch("pilo.zfs.list_snapshots", return_value=[
        "tank@20260101_000000_000000-anchor",
        "tank@20260101_010000_000000-incr",
        "tank@20260102_000000_000000-anchor",
        "tank@20260102_010000_000000-incr",
        "tank@20260103_000000_000000-anchor",
    ])
    def test_three_anchors_two_pairs(self, mock_list):
        pairs = rollups.discover_anchor_chain("tank")
        self.assertEqual(len(pairs), 2)
        self.assertEqual(pairs[0][0], "20260101_000000_000000-anchor")
        self.assertEqual(pairs[0][1], "20260102_000000_000000-anchor")
        self.assertEqual(pairs[1][0], "20260102_000000_000000-anchor")
        self.assertEqual(pairs[1][1], "20260103_000000_000000-anchor")
    ...
    def test_chronological_order(self, mock_list):
        ...
        self.assertEqual(
            pairs[1],
            ("20260102_000000_000000-anchor",
             "20260103_000000_000000-anchor"),
        )

    @patch("pilo.zfs.list_snapshots", return_value=[
        "tank@20260101_000000_000000-incr",
        "tank@20260102_000000_000000-incr",
    ])
    def test_non_anchor_ignored(self, mock_list):
        pairs = rollups.discover_anchor_chain("tank")
        self.assertEqual(pairs, [])
...
class TestExecuteRollupPlan(pilotest.TestCase):
    ...
    def test_single_op(self, mock_send, mock_guid, mock_manifest):
        op = RollupOp(
            output_path=Path("/out/r.zfs"),
            base_snapshot="tank@base-anchor",
            target_snapshot="tank@target-anchor",
            source_dataset="tank",
        )
        plan = RollupPlan(ops=(op,))
        results = list(rollups.execute_rollup_plan(plan))
        ...
        mock_send.assert_called_once_with(
            "tank@base-anchor", "tank@target-anchor",
            Path("/out/r.zfs"),
        )
        mock_guid.assert_called_once_with("tank@target-anchor")
        mock_manifest.assert_called_once_with(
            Path("/out/r.zfs"),
            snapshot_name="target-anchor",
            source="tank",
            guid="guid123",
            base_snapshot="base-anchor",
        )
